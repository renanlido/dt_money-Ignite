import { useState } from 'react';
import Modal from 'react-modal';

import { TransactionsProvider } from './hooks/useTransactions';

import { Dashboard } from './components/Dashboard';
import { Header } from './components/Header'
import { NewTransactionModal } from './components/NewTransactionModal';

import { GlobalStyle } from "./styles/global";

Modal.setAppElement('#root');

export function App() {
  const [isCreateTransactionModalOpen, setIsCreateTransactionModalOpen] = useState(false);

  function handleOpenCreateTransactionModal() {
    setIsCreateTransactionModalOpen(true);
  }

  function handleCloseCreateTransactionModal() {
    setIsCreateTransactionModalOpen(false);
  }

  return (
    <TransactionsProvider>
      <Header onOpenNewTransactionModal={handleOpenCreateTransactionModal} />
      <Dashboard />

      <NewTransactionModal 
      isOpen={isCreateTransactionModalOpen}
      onRequestClose={handleCloseCreateTransactionModal}
      />

      <GlobalStyle />
    </TransactionsProvider>
  );
}
